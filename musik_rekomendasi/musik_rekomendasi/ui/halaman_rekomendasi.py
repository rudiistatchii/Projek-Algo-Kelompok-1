...
class HalamanRekomendasi(tk.Frame):
    ...
    def _bangun_ui(self):
        # ── Header ──
        tk.Label(self, text="🔍  Rekomendasi Lagu", bg=WARNA["bg"], fg=WARNA["aksen"], font=("Consolas", 16, "bold")).pack(anchor="w", padx=15, pady=(12, 5))
        ...
        tk.Label(baris1, text="Genre:", bg=WARNA["panel"], fg=WARNA["teks2"], font=("Consolas", 10)).pack(side="left", padx=(0, 6))
        self.cb_genre = ttk.Combobox(baris1, values=GENRE_LIST, state="readonly", font=("Consolas", 10), width=14)
        self.cb_genre.current(0)
        self.cb_genre.pack(side="left", padx=(0, 20))

        tk.Label(baris1, text="Mood:", bg=WARNA["panel"], fg=WARNA["teks2"], font=("Consolas", 10)).pack(side="left", padx=(0, 6))
        self.cb_mood = ttk.Combobox(baris1, values=MOOD_LIST, state="readonly", font=("Consolas", 10), width=14)
        ...
        tk.Label(baris2, text="Cari:", bg=WARNA["panel"], fg=WARNA["teks2"], font=("Consolas", 10)).pack(side="left", padx=(0, 6))
        self.entry_cari = tk.Entry(baris2, bg=WARNA["card"], fg=WARNA["teks"], insertbackground="white", font=("Consolas", 10), relief="flat", width=30)
        self.entry_cari.pack(side="left", padx=(0, 10))

        tk.Button(baris2, text="🔎 Cari", bg=WARNA["aksen"], fg="white", font=("Consolas", 9, "bold"), relief="flat", padx=12, command=self._cari).pack(side="left", padx=4)
        tk.Button(baris2, text="Rekomendasi", bg=WARNA["hijau"], fg="#111", font=("Consolas", 9, "bold"), relief="flat", padx=12, command=self._rekomendasikan).pack(side="left", padx=4)
        tk.Button(baris2, text="Reset", bg=WARNA["card"], fg=WARNA["teks2"], font=("Consolas", 9), relief="flat", padx=8, command=self._reset).pack(side="left", padx=4)

        # ── Label info hasil ──
        self.label_info = tk.Label(self, text="", bg=WARNA["bg"], fg=WARNA["teks2"], font=("Consolas", 9))
        ...
        self.tabel = ttk.Treeview(tabel_frame, columns=cols, show="headings", height=14, selectmode="browse")

        lebar = {"ID": 40, "Judul": 200, "Artis": 140, "Genre": 70, "Mood": 80, "Durasi": 60, "Pop.": 50}
        for col in cols:
            self.tabel.heading(col, text=col)
            self.tabel.column(col, width=lebar[col], anchor="center")

        scrollbar = ttk.Scrollbar(tabel_frame, orient="vertical", command=self.tabel.yview)
        ...
        tk.Button(self, text="➕ Tambah ke Playlist", bg=WARNA["hijau"], fg="#111", font=("Consolas", 10, "bold"), relief="flat", pady=6, command=self._ke_playlist).pack(pady=(0, 10))
    ...
    def _rekomendasikan(self):
        ...
        self.label_info.config(
            text=f"Linear Search selesai — {len(hasil)} lagu ditemukan " f"(genre: {genre}, mood: {mood}), diurutkan by popularitas"
        )
    ...
    def _tampilkan(self, daftar_lagu: list):
        ...
        for l in daftar_lagu:
            self.tabel.insert("", "end", iid=str(l.id), values=(l.id, l.judul, l.artis, l.genre, l.mood, l.durasi_format(), l.popularitas))
